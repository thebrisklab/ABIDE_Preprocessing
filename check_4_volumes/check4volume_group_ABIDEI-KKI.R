...
for (sub in subnames) {
  funcname=list.files(paste0("ABIDEI-KKI/",sub,"/func/"))
  path_name=paste0("ABIDEI-KKI/",sub,"/func/",funcname)
  ...
  # png(filename = paste0("check_4_volumes/ABIDEI-KKI/",sub,"_Mean_series_plot.png"),width = 800, height = 350)
  # plot(1:(dim(fmri_image)[4]), ts_mean, type = "l",xlab = "Time",ylab = "Mean Signal",main = paste0("Time series for ",sub))
  # dev.off()
  cat(dim(fmri_image)[4],"\n")
