...
for (sub in subnames) {
  ...
  nii_index=grep("36p_spike.nii.gz",file_list)
...
# write the txt tile
setwd("/data/home4/risk_share/ImproveFConnASD/ABIDE/fmriprep_preprocessed/36p/script/cifti_based/nifti_to_cifti/KKI/36p_spike")
...
# write the txt tile
setwd("/data/home4/risk_share/ImproveFConnASD/ABIDE/fmriprep_preprocessed/36p/script/cifti_based/nifti_to_cifti/KKI/36p_spike")
write.table(my_list, file = "subjList_ABIDEII_nifti.txt",
            row.names = FALSE, col.names = FALSE,quote = FALSE)
